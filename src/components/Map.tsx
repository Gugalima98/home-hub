...
const createCustomIcon = (price: number) => {
  ...
  return L.divIcon({
    ...
    html: `<div class="bg-[#3b44c6] text-white font-bold text-xs px-2 py-1 rounded-md shadow-md border border-white whitespace-nowrap hover:scale-110 transition-transform cursor-pointer">
            ${formattedPrice}
           </div>`,
    iconSize: [80, 30],
      ...
      >
        ...
        {properties.map((property) => (
          <Marker
            ...
            icon={createCustomIcon(property.totalPrice)}
            zIndexOffset={hoveredPropertyId === property.id ? 1000 : 0}
            eventHandlers={{
                click: () => {
                    // Navigate to details or open popup
                }
            }}
          >
